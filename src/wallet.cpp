...
// Computercoin: total coins staked (non-spendable until maturity)
...
bool CWallet::ComputercoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake, targetProofOfStake))
            {
                ...
                if (fDebug && GetBoolArg("-printcoinstake"))
                    printf("ComputercoinStake : kernel found\n");
                ...
                {
                    if (fDebug && GetBoolArg("-printcoinstake"))
                        printf("ComputercoinStake : failed to parse kernel\n");
                    break;
                }
                if (fDebug && GetBoolArg("-printcoinstake"))
                    printf("ComputercoinStake : parsed kernel type=%d\n", whichType);
                if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH)
                {
                    if (fDebug && GetBoolArg("-printcoinstake"))
                        printf("ComputercoinStake : no support for kernel type=%d\n", whichType);
                ...
                if (whichType == TX_PUBKEYHASH) // pay to address type
                {
                    ...
                    {
                        if (fDebug && GetBoolArg("-printcoinstake"))
                            printf("ComputercoinStake : failed to get key for kernel type=%d\n", whichType);
                ...
                {
                    if (fDebug && GetBoolArg("-printcoinstake"))
                        printf("ComputercoinStake : invalid key for kernel type=%d\n", whichType);
                        break; // keys mismatch
                    }
                ...
                if (fDebug && GetBoolArg("-printcoinstake"))
                    printf("ComputercoinStake : added kernel type=%d\n", whichType);
                fKernelFound = true;
                break;
            }
        }
    ...
    // Calculate coin age reward
    {
        ...
        if (!txNew.GetCoinAge(txdb, nCoinAge))
            return error("ComputercoinStake : failed to calculate coin age");
    ...
    BOOST_FOREACH(const CWalletTx* pcoin, vwtxPrev)
    {
        if (!SignSignature(*this, *pcoin, txNew, nIn++))
            return error("ComputercoinStake : failed to sign coinstake");
    }

    // Limit size
    unsigned int nBytes = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
    if (nBytes >= MAX_BLOCK_SIZE_GEN/5)
        return error("ComputercoinStake : exceeded coinstake size limit");
...
// Computercoin: check 'spent' consistency between wallet and txindex
// Computercoin: fix wallet spent state according to txindex
...
// Computercoin: disable transaction (only for coinstake)
