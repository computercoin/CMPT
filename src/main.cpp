...
const string strMessageMagic = "Computercoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Computercoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Computercoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if(pindexBest->nHeight >= 201 && pindexBest->nHeight <= 8200)
...
//int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nFees, int nHeight)
{
    ...
    nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;
			...
			if(pindexBest->nHeight < YEARLY_BLOCKCOUNT)
				nRewardCoinYear = 18 * MAX_MINT_PROOF_OF_STAKE; //first year 20%
				...
				nRewardCoinYear = 5 * MAX_MINT_PROOF_OF_STAKE; //year 4 8%
...
// Computercoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes Computercoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Computercoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Computercoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Computercoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Computercoin: track money supply and mint amount info
...
// Computercoin: total coin age spent in transaction, in the unit of coin-days.
...
// Computercoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Computercoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Computercoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Computercoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Computercoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Computercoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Computercoin: check proof-of-stake
    ...
    // Computercoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Computercoin: getblocks may not obtain the ancestor block rejected
    ...
    // Computercoin: if responsible for sync-checkpoint send it
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    {
        if (wallet.ComputercoinStake(wallet, nBits, nSearchTime-nLastCoinStakeSearchTime, nFees, txCoinStake, key))
        {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Computercoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Computercoin is X13 POW and POS";
        CTransaction txNew;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("40a3f2f00fc245f390aa50b728ee93db8a8493c1b21f6355a963280d0f8ee14d"));
        block.print();
        ...
        // Computercoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Computercoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Computercoin: tell downloading node about the latest block if it's
